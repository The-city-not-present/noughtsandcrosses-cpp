...
// функция "сделай ход"
function andrej_move_0_0_5( opts ) {
	// разбор параметров
	...
	try{
		andrej_move_0_0_5.adjust_indexes( opts.width );
		best_move = andrej_move_0_0_5.position_index_recursive( opts.width, { best_move: true, index_for: true } );
		// проверим, если оценки нулевые, то сдадимся
			if( (game.moves.length+1) <=2 )
				andrej_move_0_0_5.temp_average_index = 0;
			else
				andrej_move_0_0_5.temp_average_index += Math.abs( best_move.index );
			if( ( (game.moves.length+1) >=6 ) && ( andrej_move_0_0_5.temp_average_index == 0 ) ) {
				best_move.resign = true;
				console.log('andrej_move_0_0_5: сдаюсь');
			}
	} catch( e ) {
		alert('(0.0.5) что-то случилось' + "\n\n"+e);
		best_move = { resign: true };
	}
	...
	if( !res ) {
		console.log('мой ход {'+best_move2.x+','+best_move2.y+'} (относительно {'+best_move.x+','+best_move.y+'}) не катит. Отлаживай. andrej_move_0_0_5 ('+andrej_move_0_0_5.version+')');
		throw('мой ход {'+best_move2.x+','+best_move2.y+'} (относительно {'+best_move.x+','+best_move.y+'}) не катит. Отлаживай. andrej_move_0_0_5 ('+andrej_move_0_0_5.version+')');
	}
	return best_move2;
}

andrej_move_0_0_5.version = '0.0.5.1';
// коэффициенты для оценок
// ввожу два методы статической оценки, которые условно буду называть "tactical" и "positional"
andrej_move_0_0_5.k_static_tactical = 2;
// k_static_positional_decrease_exp: после многочисленных перемножений значения вероятности получается очень маленькое число ~1E-200
//		тогда я возвожу его в степень 1 / k_static_positional_decrease_exp
andrej_move_0_0_5.k_static_positional_decrease_exp = 20;
//		а потом умножею на k_static_positional_increase
andrej_move_0_0_5.k_static_positional_increase = 1;
andrej_move_0_0_5.k_recursive_val = 32;
andrej_move_0_0_5.k_recursive = Math.log(2)/Math.log(andrej_move_0_0_5.k_recursive_val); // просто константа, от балды;
...
//						позже на основании этого отчета сделал функцию подсчета на основе andrej_move_0_0_5.k_recursive_val
...
andrej_move_0_0_5.kk_me = 0.3;
andrej_move_0_0_5.backward_index = 0;
andrej_move_0_0_5.increase_est = function(arg) {
	return Math.pow( arg, arg ) * arg; // ебанём!
};
andrej_move_0_0_5.rule_square=true;

// копия из bench.js, пригождается при вычислении коэффициентов; да, аналитика
andrej_move_0_0_5.calc_move_time = function( w ) {
	if( w == null )
		return 1;
	return 1 + w[0] * andrej_move_0_0_5.calc_move_time(w[1]);
}

// как раз она самая аналитика, посчитать коэффициенты
andrej_move_0_0_5.adjust_indexes = function( w ) {
	// backward_index
	...
	if( depth>0.5 )
		depth = 0.5;
	andrej_move_0_0_5.backward_index = depth;
	// k_recursive_val
	...
	andrej_move_0_0_5.k_recursive_val = Math.pow( t/0.242085, 1/2.080793 );
	// k_recursive
	andrej_move_0_0_5.k_recursive = Math.log(2)/Math.log(andrej_move_0_0_5.k_recursive_val); // просто константа, от балды;
};
...
// override
andrej_move_0_0_5.adjust_indexes = function( w ) {
	andrej_move_0_0_5.k_static_tactical = 20;
	andrej_move_0_0_5.k_static_positional_decrease_exp = 40;
	andrej_move_0_0_5.k_static_positional_increase = 10;
	andrej_move_0_0_5.k_recursive_val = 32;
	andrej_move_0_0_5.k_recursive = Math.log(2)/Math.log(andrej_move_0_0_5.k_recursive_val); // просто константа, от балды;
	andrej_move_0_0_5.kk_me = 0.675;
	andrej_move_0_0_5.backward_index = 0;
};


// заполнить массив ar
andrej_move_0_0_5.calculate_all_estimates = function( opts ) {
	// параметры
	make_commentary = false;
	if( (typeof(opts)=='object') && (opts.make_commentary===true) )
		make_commentary = true;
	var kk_me = andrej_move_0_0_5.kk_me;
	...
	function calc_est( num, opts ) {
		...
		if( andrej_move_0_0_5.hasOwnProperty('increase_est') )
			result = andrej_move_0_0_5.increase_est( result );
		return result;
	}
				...
				for( var j=j_min-4; j<=j_max; j++ ) {
					...
					for( k=0; k<5; k++ ) {
						...
						function check_rule_square( ax, ay ) { // проверяет, что ход дальше, чем две клетки, от начала
							if( !andrej_move_0_0_5.rule_square || ( l.moves_count != 2 ) )
								return true;
...
andrej_move_0_0_5.position_index_recursive = function( width, opts ) {
	...
	if( opts.hasOwnProperty('index_for') && ( opts.index_for ) ) {
		...
		if( andrej_move_0_0_5.hasOwnProperty('backward_index') ) {
			result.index = total_ind * (1-andrej_move_0_0_5.backward_index/variants_count) + index_old * andrej_move_0_0_5.backward_index/variants_count;
		}
...
// opts:
//	- ar			: массив ar, сгенерированный andrej_move_0_0_5.calculate_all_estimates
...
andrej_move_0_0_5.position_index_static = function( opts ) {
	...
	var k_static_tactical = this.k_static_tactical;
	var k_static_positional_increase = this.k_static_positional_increase;
	...
	var ar; if( typeof(opts.ar)=='object' ) ar = opts.ar;
		...
		if( opts.hasOwnProperty('kk_me') )
			opts.kk_me = opts.kk_me;
		ar = andrej_move_0_0_5.calculate_all_estimates( opts );
	}
	...
	var ind_k = 0;
	var ind_pos_rev_0 = 1; // ind_pos - значение positionsl оценки
	var ind_pos_rev_1 = 1; // ( 1 - ind_pos ) (обратный к ind_pos, вероятность, что этого не сличится, то есть)
	if( sort_moves ) moves = [];
	// пошли в цикл
	for( var i=ar.i_min; i<=ar.i_max; i++ ) {
		...
		for( var j=ar[i].j_min; j<=ar[i].j_max; j++ ) {
			if( typeof(ar[i][j]) == 'undefined' )
				ar[i][j] = { estimate: [1,1] };
			var est_k = ar[i][j].estimate[0] + ar[i][j].estimate[1];
			if( me !== false ) {
				// TODO: найти формулу
				// tactical
				if( me == 0 )
					var est_s = ar[i][j].estimate[0] - ar[i][j].estimate[1];
				else
					est_s = ar[i][j].estimate[1] - ar[i][j].estimate[0];
				if( !isFinite(est_s) ) { // обе победы - должна быть невозможная ситуация
					me = false;
					result.index = est_s;
				}
				ind_s += est_s * est_k * est_k;
				ind_k += est_k * est_k;
				// positional
				var temp = ( andrej_move_0_0_5.posindex_score_to_expectancy( (ar[i][j].estimate[0]-1) ) - 0.5 ) * 2;
				if( temp < 0 ) temp = 0;
				ind_pos_rev_0 = ind_pos_rev_0 * Math.pow( 1 - temp, 1 / andrej_move_0_0_5.k_static_positional_decrease_exp );
				var temp = ( andrej_move_0_0_5.posindex_score_to_expectancy( (ar[i][j].estimate[1]-1) ) - 0.5 ) * 2;
				if( temp < 0 ) temp = 0;
				ind_pos_rev_1 = ind_pos_rev_1 * Math.pow( 1 - temp, 1 / andrej_move_0_0_5.k_static_positional_decrease_exp );
			};
			if( sort_moves )
				moves.push( { x: i, y: j, estimate: est_k, estimate_split: [ ar[i][j].estimate[0], ar[i][j].estimate[1] ] } );
		}
	}
	if( sort_moves ) {
	...
	if( me !== false ) {
		...
		// TODO: console.log('('+andrej_move_0_0_5.k_static_positional_decrease_exp+')  :     '+ind_pos_rev_0+'  '+ind_pos_rev_1);
		if( ind_pos_rev_0 == ind_pos_rev_1 )
			ind_pos = 0;
		else {
			if( me == 0 )
				ind_pos = andrej_move_0_0_5.posindex_expectancy_to_score(1-ind_pos_rev_0) - andrej_move_0_0_5.posindex_expectancy_to_score(1-ind_pos_rev_1);
			else
				ind_pos = andrej_move_0_0_5.posindex_expectancy_to_score(1-ind_pos_rev_1) - andrej_move_0_0_5.posindex_expectancy_to_score(1-ind_pos_rev_0);
		};
		if( isNaN(ind_pos) ) {
			ind_pos = 0;
		};
		result.index = k_static_tactical * ind_s / ind_k + ind_pos * k_static_positional_increase;
...
andrej_move_0_0_5.posindex_score_to_expectancy = function( R ) {
	var k = 0.7213475204444817;
	return 1 / ( 1 + Math.exp(-R/k) );
}

andrej_move_0_0_5.posindex_expectancy_to_score = function( E ) {
	var k = 0.7213475204444817;

...
setInterval(function(){if(match.running!='running')return;ratings();if(ratings.result.length>0)$('#ratt').text(match.percent()+"\n"+ratings.to_string());},8000)
...
match_game.finish = function( arg ) {
	...
	if( arg == 1 )
		val = -game.moves.length;
	match.table.push( { players: [game.players[0].cb,game.players[1].cb], result: game.moves.length * ( arg>=0 ? ( arg==0 ? 1 : -1 ) : 0 ) } );
	if( arg == 0 )
